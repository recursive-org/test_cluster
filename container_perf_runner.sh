...
SBATCH_JOB_ID=""
PHASE=${PHASE:-"run"}  # run|start|bench|cleanup
STATE_FILE=${STATE_FILE:-""}  # where to write connection state in start phase
SKIP_CLEANUP=${SKIP_CLEANUP:-0}  # set to 1 to keep service alive on exit
...
cleanup_on_exit() {
  if [[ "${SKIP_CLEANUP}" == "1" || "${PHASE}" == "start" || "${PHASE}" == "bench" ]]; then
    return 0
  fi
  do_cleanup_now
}
...
write_state_file() {
  ...
  {
    echo "# Generated by container_perf_runner.sh at $(date -Iseconds)"
    printf 'RUN_MODE=%q\n' "${RUN_MODE}"
    printf 'MODEL_PATH=%q\n' "${MODEL_PATH}"
    printf 'TP=%q\n' "${TP}"
    printf 'PORT=%q\n' "${PORT}"
    printf 'SERVICE_HOST=%q\n' "${SERVICE_HOST}"
    printf 'MEM_FRACTION_STATIC=%q\n' "${MEM_FRACTION_STATIC}"
    printf 'IMAGE=%q\n' "${IMAGE}"
    printf 'HF_CACHE=%q\n' "${HF_CACHE}"
    printf 'CONTAINER_NAME=%q\n' "${CONTAINER_NAME}"
    printf 'SBATCH_SCRIPT=%q\n' "${SBATCH_SCRIPT}"
    printf 'SBATCH_JOB_ID=%q\n' "${SBATCH_JOB_ID}"
  } > "${path}"
}

resolve_sbatch_script() {
  if [[ -n "${SBATCH_SCRIPT}" ]]; then
    return 0
  fi

  local candidate="${SCRIPT_DIR}/launch.sbatch"
  if [[ -f "${candidate}" ]]; then
    SBATCH_SCRIPT="${candidate}"
    return 0
  fi

  candidate="${HOME}/test_cluster/launch.sbatch"
  if [[ -f "${candidate}" ]]; then
    SBATCH_SCRIPT="${candidate}"
    return 0
  fi

  # Set a sensible default for error messages.
  SBATCH_SCRIPT="${SCRIPT_DIR}/launch.sbatch"
}
...
if [[ "${PHASE}" == "start" ]]; then
  ...
  if [[ -n "${STATE_FILE}" ]]; then
    echo "Wrote service state to ${STATE_FILE}"
  fi
  exit 0
fi
...
PY
  ...
  STARTUP_WAIT_SEC=$(START_TIME="${startup_start}" END_TIME="${startup_end}" python3 - <<'PY'
import os
